...
async def price_oracle(
    ...
    condition: str = Query(..., pattern=r"^(new|like_new|good|fair|for_parts)$"),
    brand: str | None = Query(default=None),
    redis: Redis = Depends(get_redis),
):
    """Fair value estimate from comparable completed auctions.

    FR-AI-001: Queries ClickHouse for comparables (same category,
    similar condition, sold in last 90 days). Returns price range,
    suggested starting price, and confidence level.
