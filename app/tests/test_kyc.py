...
Covers:
  ...
  POST /auth/kyc/submit — three confidence branches:
    - >= 85%: auto-approve → status=verified
    ...
    - Second attempt after rejection
    - Max attempts exhausted

  Admin endpoints:
    - GET /admin/kyc/queue: lists pending reviews
    - POST /admin/kyc/{user_id}/approve: approves + fires notification
    - POST /admin/kyc/{user_id}/reject: rejects + fires notification
...
def _make_token(user_id: str, role: str = "buyer", kyc: str = "pending") -> str:
...
class TestKYCEligibility:
    async def test_pending_user_eligible(self, db_session):
        user = await _create_user(db_session)
        ok, err = await kyc_service.check_kyc_eligibility(user)
        assert ok is True
        assert err is None
    ...
    async def test_max_attempts_not_eligible(self, db_session):
        user = await _create_user(db_session, attempts=2)
        ok, err = await kyc_service.check_kyc_eligibility(user)
        assert ok is False
...
class TestKYCSubmit:
    """Tests the core submit_kyc logic by mocking Rekognition."""

    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_auto_approve_above_85(self, mock_rek_fn, db_session):
        """PM-02: >= 85% → KYC_VERIFIED immediately."""
        mock_rek_fn.return_value = _mock_rekognition(92.5)
        user = await _create_user(db_session)

        result = await kyc_service.submit_kyc(
            user, "kyc/id_front.jpg", "kyc/id_back.jpg", "kyc/selfie.jpg", db_session,
        )
        ...
        assert user.kyc_attempt_count == 1

    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_manual_review_70_to_84(self, mock_rek_fn, db_session):
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_reject_below_70(self, mock_rek_fn, db_session):
    ...
    async def test_reject_max_attempts_exhausted(self, mock_rek_fn, db_session):
        ...
        assert user.kyc_attempt_count == 2

    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_rekognition_unavailable_queues_manual(self, mock_rek_fn, db_session):
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_exactly_85_auto_approves(self, mock_rek_fn, db_session):
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_exactly_70_manual_review(self, mock_rek_fn, db_session):
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_documents_stored_in_db(self, mock_rek_fn, db_session):
        """Three KYCDocument rows created per submission."""
        mock_rek_fn.return_value = _mock_rekognition(90.0)
        user = await _create_user(db_session)

        await kyc_service.submit_kyc(
            user, "kyc/front.jpg", "kyc/back.jpg", "kyc/self.jpg", db_session,
        )

        from sqlalchemy import select
        from app.services.auth.models import KYCDocument
...
class TestAdminReview:
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_approve_sets_verified(self, mock_rek_fn, db_session):
        ...
        await kyc_service.submit_kyc(
            user, "kyc/f.jpg", "kyc/b.jpg", "kyc/s.jpg", db_session,
        )
        assert user.kyc_status == KYCStatus.PENDING_REVIEW
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_reject_sets_rejected(self, mock_rek_fn, db_session):
...
class TestKYCSubmitEndpoint:
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_auto_approve(self, mock_rek_fn, client, db_session, fake_redis):
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_manual_review(self, mock_rek_fn, client, db_session, fake_redis):
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_rejection(self, mock_rek_fn, client, db_session, fake_redis):
...
class TestAdminKYCEndpoints:
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_queue_returns_pending_items(self, mock_rek_fn, client, db_session, fake_redis):
        mock_rek_fn.return_value = _mock_rekognition(75.0)
        user = await _create_user(db_session)
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_approve_endpoint(self, mock_rek_fn, client, db_session, fake_redis):
        mock_rek_fn.return_value = _mock_rekognition(75.0)
        user = await _create_user(db_session)
        admin = await _create_user(
            db_session, phone="+962790000001", role=UserRole.ADMIN,
    ...
    @patch("app.services.auth.kyc_service._get_rekognition_client")
    async def test_reject_endpoint(self, mock_rek_fn, client, db_session, fake_redis):
    ...
    async def test_approve_nonexistent_user_returns_404(
        self, mock_rek_fn, client, db_session, fake_redis,
    ):
        admin = await _create_user(
            db_session, phone="+962790000001", role=UserRole.ADMIN,
        )
